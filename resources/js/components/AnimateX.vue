<template>
    <div style="row padding:80px">
        ...
        <div class="col-md-6 offset-3 my-4">
            <div class="row justify-content-start" v-if="drawTypeId!=3">
                <div v-for="(category,index) in categories" :key="index" class="col-md-3 offset-1 category"
                    :class="{selected_category:completedCategories.includes(category.id)}" @click="setSelectedCategory(category)">
        ...
        <div class="col-md-6 offset-3 text-center">
            ...
            <div v-if="drawTypeId!=3">
                <button  v-if="!isloading && !hasCompletedDraw" class="btn spin-button mt-5" :class="{none: isloading && hasCompletedDraw}" @click.prevent="spin()">Spin</button>
                <p v-if="isloading && !hasCompletedDraw">finding the lucky winner...</p>

                <button v-if="hasCompletedDraw" class="btn spin-button btn-complete" @click="exportAndCompleteDraw()">Export and Complete Draw</button>
            </div>
            <div v-if="drawTypeId == 3">
                <button  v-if="!isloading && !setGrandWinner" class="btn spin-button mt-5" data-backdrop="static" data-keyboard="false" @click.prevent="spin()">Spin</button>
                <p v-if="isloading && !setGrandWinner">finding the lucky winner...</p>

                <button v-if="setGrandWinner" class="btn spin-button btn-complete" @click="exportAndCompleteDraw()">Export and Complete Draw</button>
            </div>
        ...
        <div class="col-md-8 offset-2 mt-5">
            <table class="table table-striped table-sm">
                ...
                <tbody v-if="!isloading && isGrandWinner && setGrandWinner">
                    <tr class="slist-item">
                        <td>{{1}}</td>
                        <td>{{phone_number}}</td>
                        <td>{{winner_name}}</td>
            ...
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content">
                    <div class="modal-body jackpot-modal-content">
                        <p class="title">{{ winnerCategory.name }}</p>
                        <img v-if="!isGrandWinner" :src="'../images/'+winnerCategory.image" style="height:190px;width:auto;object-fit:contain;">
                        <img v-if="isGrandWinner" :src="'../images/grand_prize.png'" style="height:190px;width:auto;object-fit:contain;">
                        <p class="winner-number">{{winner_name}}</p>
                        <p class="winner-category" v-if="!isGrandWinner">Winner - {{phone_number}}</p>
                        <p class="winner-category" style="line-height:40px" v-if="isGrandWinner">Grand Winner - {{phone_number}}</p>
                        <div class="row" v-if="!isloading">
                            <div class="col-md-6">
                                <button type="button" class="btn btn-outline-danger mt-5 btn-complete" @click="discardAndContinue()">Disqualify</button>
                            </div>
                            <div class="col-md-6">
                                <button type="button" class="btn btn-primary mt-5" @click="saveWinner()">Qualify</button>
                            </div>
                        </div>
            ...
            this.axios.post('spin',body).then(response=>{
                if(response.status == 200){
                    if(response.data['category'] == 'grand'){
                        ...
                        this.winnerCategory = response.data['category']
                    }

                    var account_number = response.data['phone_number']
                    this.account_number = account_number
                    this.phone_number = response.data['phone_number']
                    this.winner_name = response.data['winner_name']
                    this.toArray(account_number)
